...
public class QInverted {
static Inverted inv;
	public QInverted(Inverted inv) {
		this.inv = inv;
	...
	public static  OurLinkedList<Integer> BooleanQuery( String Q) {
	...
	public static OurLinkedList<Integer> And(String Q) {
		...
		boolean exist = inv.searchInverted(Split[0].trim().toLowerCase());
		
		if(exist) {
			L1 = inv.invertedIndex.retrieve().doc_IDS;
		}
		
		for(int i = 1; i < Split.length; i++) {
			
			 exist = inv.searchInverted(Split[i].trim().toLowerCase());
			 
			if(exist) {
				L2 = inv.invertedIndex.retrieve().doc_IDS;
